...
vault_backtest.py
─────────────────
Rapid vault EV strategy backtest against the last N days of graded predictions.

Pulls predictions + grading caches (already on disk from audit runs), applies
the exact vault rules (1/8 Kelly, 2% cap, max 7/day, compounding), and prints
a full performance report.

Usage:
    python vault_backtest.py --days 14
    python vault_backtest.py --days 30
"""
...
# ── Vault Constants (matching kelly_optimizer.py + VaultTab.tsx) ───────────
KELLY_FRACTION = 0.125  # 1/8th Kelly
MAX_STAKE_PCT = 0.02    # 2% hard cap
FRAGILE_MAX = 0.01       # 1% for fragile markets
WATCH_MAX = 0.015        # 1.5% for watch markets
MAX_DAILY_PICKS = 7
LAGOS_TZ = timezone(timedelta(hours=1))

FRAGILE_MARKETS = {"home_win", "away_win", "draw", "btts_no", "over35", "under35", "double_chance_x2"}
WATCH_MARKETS = {"under25", "under35", "double_chance_x2", "draw_no_bet_away"}
...
def market_max_stake(market: str, tier: str = "stable") -> float:
    ...
    if t == "watch":
        return WATCH_MAX
    m = (market or "").lower()
    for kw in ["away win", "draw", "btts no", "over 3.5", "over 4.5", "under 0.5"]:
        if kw in m:
            return FRAGILE_MAX
    for kw in ["under 1.5", "under 2.5", "under 3.5", "double chance (x2)", "draw no bet (away)"]:
        if kw in m:
            return WATCH_MAX
...
def resolve_market_for_grade(market: str, hg: int, ag: int) -> str:
    ...
    if "under 3.5" in m:
        return "won" if total < 4 else "lost"
    if "under 4.5" in m:
        return "won" if total < 5 else "lost"
...
def run_vault_backtest(days: int = 14, bankroll: float = 10000):
    """Run full vault EV backtest against graded Firestore predictions."""
    ...
    print(f"\n{'='*70}")
    print(f"  VAULT EV STRATEGY BACKTEST — {days} DAYS")
    print(f"  Starting Bankroll: {bankroll:,.0f} FCFA")
    print(f"  Strategy: 1/8 Kelly (0.125x), 2% hard cap, max {MAX_DAILY_PICKS} picks/day")
    print(f"  Period: {dates[0]} to {dates[-1]}")
    print(f"{'='*70}\n")

    # ── Pull predictions + results ─────────────────────────────────────────
    all_daily_picks = []
    daily_log = []
    market_stats = defaultdict(lambda: {"picks": 0, "wins": 0, "losses": 0, "voids": 0, "staked": 0.0, "returned": 0.0})
    tier_stats = defaultdict(lambda: {"picks": 0, "wins": 0, "losses": 0, "voids": 0})
    ...
    for date_str in dates:
        ...
        for pred in predictions:
            ...
            kelly = kelly_stake_pct(prob, odds, market, calib_tier)
            if kelly <= 0:
                continue
        ...
        # Sort: category priority > composite quality
        tier_priority = {"safe": 2, "value": 1}
        candidates.sort(key=lambda c: (
            tier_priority.get(c["category"], 0),
        ...
        for c in candidates:
            ...
            # Tier stats
            ts = tier_stats[c["calibration_tier"]]
            ts["picks"] += 1
            if grade == "won": ts["wins"] += 1
            elif grade == "lost": ts["losses"] += 1
            else: ts["voids"] += 1
        ...
        if day_picks:
            ...
            print(f"  {date_str}: {len(day_picks)} picks | P/L: {day_profit:+,.0f} FCFA | Bankroll: {bankroll:,.0f}")
    ...
    bankroll_t = 10000.0
    for dl in daily_log:
        ret = dl["profit"] / max(bankroll_t, 1)
        daily_returns.append(ret)
        bankroll_t = dl["bankroll"]
    ...
    if market_stats:
        print(f"\n  {'MARKET':<28} {'Picks':>5} {'W':>4} {'L':>4} {'Hit%':>7} {'Staked':>10} {'ROI':>8} {'Flag'}")
        print(f"  {'-'*28} {'-'*5} {'-'*4} {'-'*4} {'-'*7} {'-'*10} {'-'*8} {'-'*4}")
        for mkt, ms in sorted(market_stats.items(), key=lambda x: x[1]["picks"], reverse=True):
            ...
            flag = " ⚠️" if hr < 0.45 and n >= 5 else ""
            print(f"  {mkt[:28]:<28} {ms['picks']:>5} {w:>4} {l:>4} {hr:>6.1%} {ms['staked']:>10,.0f} {mkt_roi:>+7.1%}{flag}")

    # Per-tier breakdown
    print(f"\n  {'CALIBRATION TIER':<20} {'Picks':>5} {'W':>4} {'L':>4} {'Hit%':>7}")
    print(f"  {'-'*20} {'-'*5} {'-'*4} {'-'*4} {'-'*7}")
    for tier, ts in sorted(tier_stats.items()):
        w, l = ts["wins"], ts["losses"]
        n = w + l
        if n == 0:
            continue
        print(f"  {tier[:20]:<20} {ts['picks']:>5} {w:>4} {l:>4} {w/n:>6.1%}")
    ...
    # Verdict
    verdict = "PROFITABLE" if bankroll > 10000 else ("NEEDS WORK" if bankroll < 8500 else "MARGINAL")
    print(f"\n{'='*70}")
    print(f"  VERDICT: {verdict}")
    print(f"  Bankroll: 10,000 → {bankroll:,.0f} FCFA ({pct_return:+.1f}%) in {dates_with_picks} trading days")
    print(f"{'='*70}\n")
...
if __name__ == "__main__":
    ...
    run_vault_backtest(days=args.days, bankroll=args.bankroll)
